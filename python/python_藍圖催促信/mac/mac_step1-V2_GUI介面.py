...
# 載入 Email 資料並合併
print("請選擇 Email 資料檔案...")
file_email_data = filedialog.askopenfilename(
    title="選擇 Email 資料檔案",
    filetypes=[("Excel files", "*.xlsx;*.xls")]
)

if not file_email_data:
    print("No email data file selected")
    exit()
...
# 透過 GUI 介面選擇檔案存放位置
print("請指定輸出檔案存放處...")
output_dir = filedialog.askdirectory(title="請指定輸出檔案存放處")
...
for i, start_row in enumerate(range(0, formatted_data.shape[0], chunk_size)):
    ...
    file_name = f'Formatted_Employee_Training_Data_Chunk_{i+1}_{current_datetime}.xlsx'
    # 使用 os.path.join 結合使用者選擇的目錄與檔案名稱
